...
public final class StorageUtils {
    ...
    private static File getCacheDirectory(Context context, boolean preferExternal) {
        ...
        if (appCacheDir == null) {
            ...
            Log.w(ProxyCacheUtils.LOG_TAG, "Can't define system cache directory! '" + cacheDirPath + "%s' will be used.");
...
//        File dataDir = new File(new File(Environment.getExternalStorageDirectory(), "Android"), "data");
        File appCacheDir = context.getExternalFilesDir(Environment.DIRECTORY_MUSIC);
//                new File(new File(dataDir, context.getPackageName()), "cache");
        if (!appCacheDir.exists()) {
            if (!appCacheDir.mkdirs()) {
                Log.w(ProxyCacheUtils.LOG_TAG, "Unable to create external cache directory");
                return null;
            }
        }
        return appCacheDir;
