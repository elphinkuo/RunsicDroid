...
public class ArcProgress extends View {
    ...
    private RectF rectDark = new RectF();
    private float fmRadius = 0;
    ...
    //logo色 #1ed760
    private final int default_finished_color = Color.rgb(30, 215, 96);
    //未完成颜色    #0b3018
    private final int default_unfinished_color = Color.rgb(11, 48, 24);
...
//        rectF.set(strokeWidth / 2f, strokeWidth / 2f, width - strokeWidth / 2f, MeasureSpec.getSize(heightMeasureSpec) - strokeWidth / 2f);
        ...
        fmRadius = (float)(width / (2 * (Math.sin(0.1*Math.PI))));
        float r = fmRadius;
        if (DEBUG) {
            Log.d("FM", "Radius is " + r);
        }
        rectF.set((width/2 - r) - strokeWidth, 0 + strokeWidth, (width/2 + r) - strokeWidth, 2*r - strokeWidth);
//        rectF.set(0, 0, width, MeasureSpec.getSize(heightMeasureSpec) /2f);

        if (DEBUG) {
            Log.d("FM", "onMeasure: " + rectF);
        }
//        rectF.set(0, 0, 600, 200);
...
//        float arcAngle = 180;
        ...
        if (DEBUG) {
            Log.d("FM", "finishedStartAngle is" + finishedStartAngle);
        }
        //画透明遮罩部分 黑色 透明度60
        paint.setColor(Color.BLACK);
        paint.setAlpha(60);
        paint.setStyle(Paint.Style.FILL);
        canvas.drawCircle(width / 2, fmRadius, fmRadius - strokeWidth * 0.7f, paint);
        if (DEBUG) {
            Log.e("FM", "center point is" + width / 2 + " " + fmRadius);
        }
        //画弧线 未完成部分
        initPainters();
